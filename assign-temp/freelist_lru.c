...
void moveToFirst(Buffer buffer_id);
void pop();
void push(Buffer buffer_id);
...
LRUNode* removeFirst() {
    ...
    LRUNode* firstNode = lruStack->head->next;
...
void moveToFirst(Buffer buffer_id) {
    LRUNode* bufferNode = lruStack->LRUNode_map[buffer_id];
    removeNode(bufferNode);
    addFirst(bufferNode);
}
...
void push(Buffer buffer_id) {
    LRUNode* node = lruStack->LRUNode_map[buffer_id];
    addFirst(node);
}
...
{
	SpinLockAcquire(&lruStack->lruStackLock);
    moveToFirst(buf_id);
    if(delete) pop();
    SpinLockRelease(&lruStack->lruStackLock);
}
...
StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state, bool *from_ring)
{
	...
	*from_ring = false;
	...
	SpinLockAcquire(&lruStack->lruStackLock);
    LRUNode* currNode = lruStack->tail;
    while(true) {
        if(currNode == lruStack->head) break;
        currNode = currNode->prev;
        buf = GetBufferDescriptor(currNode->buffer_id);
        local_buf_state = LockBufHdr(buf);
        if (BUF_STATE_GET_REFCOUNT(local_buf_state) == 0)
        {
            if (strategy != NULL) moveToFirst(currNode->buffer_id);

            *buf_state = local_buf_state;
            // remember to release lock before return
            SpinLockRelease(&lruStack->lruStackLock);
            return buf;
        }
        UnlockBufHdr(buf, local_buf_state);
    }
    // this means we have scanned all buffers
    if(currNode == lruStack->head) {
        UnlockBufHdr(buf, local_buf_state);
        //SpinLockRelease(&lruStack->lruStackLock);
    }
    SpinLockRelease(&lruStack->lruStackLock);
...
StrategyInitialize(bool init)
{
	...
	bool isLruStackExist;
    bool isHeadExist;
    bool isTailExist;
    lruStack = (LRUStack*)ShmemInitStruct("Buffer LRU Stack", sizeof(LRUStack), &isLruStackExist);
    ...
    for (int i = 0; i < NBuffers; i++) {
        ...
        lruStack->LRUNode_map[i] = (LRUNode *) ShmemInitStruct(str, sizeof(LRUNode), &isNodeExist);
    }
}
